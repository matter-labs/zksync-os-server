...
| `/status` `consensus.chain_fingerprint` | identical on every node (also logged once at startup: "committee-uniform configuration and consensus-era fingerprint") | two nodes disagree on a committee-uniform fact or resolved chain identity — schedule, observer admission, epoch geometry, consensus timing, a verification-pinned chain constant, or the actual anchor-derived consensus era. Symptoms otherwise surface later and expensively: a stall at the next epoch boundary, or false byzantine alarms | compare the drifted node's config and anchor block against a healthy node *before* the next epoch boundary; the fingerprinted surface is enumerated in `node/bin/src/chain_fingerprint.rs` |
...
2. **Verify the committee is uniform** before the flip: every node serves the
   ...
   of the fingerprinted config-and-era surface, so a half-flipped committee
   shows two fingerprints — the drift alarm doubles as flag-day progress
   tracking).
