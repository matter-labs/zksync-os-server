...
If you do any change to era-contracts, we should update zkos-l1-state.json.gz (especially if this is a breaking change -- be careful with those when we're in production).
...
* Step 5: check that everything works -- you should be able to run anvil with the new state (`gzip -dfk ./local-chains/v30.2/default/zkos-l1-state.json.gz && anvil --load_state ./local-chains/v30.2/default/zkos-l1-state.json`) and zksync-os-server **with new genesis.json** (it normally loads it from local directory).
