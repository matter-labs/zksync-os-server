...
This repo includes a pre-setup L1 state `l1-state.json` that can be loaded into `anvil`. The state was generated by `zkstack init` and essentially consists of all L1 contracts deployed and initialized with L2 genesis. 
...
It also comes with a deposit transaction that makes `0x36615cf349d7f6344891b1e7ca7c72883f5dc049` into a rich account (>10k ETH) (to regenerate it, see "Regenerate l1-state.json` below)
...
L1 state is checked in into this repo under `l1-state.json`. To regenerate it from scratch, run the following commands:
```
anvil -m "stuff slice staff easily soup parent arm payment cotton trade scatter struggle" --state ./local-chains/v30.2/l1-state.json
```
...
```
Now stop anvil (ctrl+c) - the state will be saved to the file. Rerun it with `--load-state ./local-chains/v30.2/l1-state.json`  (`--load-state` - not `--state`, otherwise it will be overwritten). Commit the new file in git.
