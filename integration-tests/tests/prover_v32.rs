...
//!   V32_FRI_PROVER_BIN        path to the `zksync_os_fri_prover` binary (may be a wrapper script)
//!   V32_APP_BIN               path to the v32 `multiblock_batch.bin` app binary
//! Optional:
//!   V32_PROVING_TIMEOUT_SECS  how long to wait for the real proof (default 4h; CPU proving is slow)
//!   V32_PROVER_CPU_THREADS    forwarded as `--cpu-worker-threads` (bounds prover memory)
...
#[ignore = "requires an externally built v32 zksync_os_fri_prover binary; run manually"]
async fn v32_native_pig_real_fri_proof_e2e() -> anyhow::Result<()> {
    let prover_bin = std::env::var("V32_FRI_PROVER_BIN")
        .expect("set V32_FRI_PROVER_BIN to the zksync_os_fri_prover binary path");
    let app_bin =
        std::env::var("V32_APP_BIN").expect("set V32_APP_BIN to the v32 multiblock_batch.bin path");
    let proving_timeout = Duration::from_secs(
        std::env::var("V32_PROVING_TIMEOUT_SECS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(4 * 3600),
    );
    let cpu_worker_threads = std::env::var("V32_PROVER_CPU_THREADS").ok();
    ...
    // Upgrade v31.0 -> v32.0 (execution V7 / zksync-os 0.4.x proving stack).
    {
    ...
    // The tx we expect to finalize through a real v32 FRI proof. Poll for the receipt
    ...
    tracing::info!(
        prover_api_url,
        block_number,
        "starting external v32 FRI prover"
    );
    ...
    loop {
        ...
        if finalized >= Some(block_number) {
            tracing::info!(
                ?finalized,
                block_number,
                "probe tx block finalized on L1 via real v32 FRI proof"
            );
            break;
        }
        anyhow::ensure!(
