...
use zksync_os_integration_tests::{
    CURRENT_TO_L1, SettlementLayer, TestCase, TestEnvironment, Tester, test_multisetup,
...
async fn v32_1_direct_l1_main_node_creates_and_commits_batch() -> anyhow::Result<()> {
    ...
    let l1_state = wait_for_l1_state(
        &tester,
        "the direct V32.1 L1 test batch to be committed on L1",
        |state| state.last_committed_batch >= batch_number,
    )
    .await?;
    ...
    assert!(
        l1_state.last_committed_batch > initial_committed_batch,
        "expected committed batch count to advance after producing a direct V32.1 batch \
         ({initial_committed_batch} -> {})",
        l1_state.last_committed_batch,
    );
