...
impl TestCase {
    ...
    /// The next protocol version on the multiprover L1. Its verifier accepts a
    /// combined Airbender + ZiSK proof only, so a real settlement there proves
    /// both lanes agreed. Fake proofs settle as they do on the default chain.
    pub const fn next_to_multiprover_l1() -> Self {
        Self {
...
pub const NEXT_TO_MULTIPROVER_L1: TestCase = TestCase::next_to_multiprover_l1();
...
pub async fn zisk_lane_status(
    prover_api_url: &str,
) -> anyhow::Result<zisk_prover_lane::handlers::ZiskLaneStatusPayload> {
