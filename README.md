...
| **L1Watcher**                                                            | Gapless list of Priority transactions - starting from the last committed to L1                               | none (TODO - temporary stores the last processed L1 block)                                                           | none (TODO - will load committed priority transactions from L1)                                                                                                                                                                                                                                                                                                         |
...
| **State**                                                                | All Storage Logs and Preimages for `blocks_to_retain_in_memory` last blocks                                  | Compacted state at some older block (`highest_block - blocks_to_retain_in_memory`): full state map and all preimages | none - `starting_block` is chosen so that this component recovers naturally                                                                                                                                                                                                                                                                                             |
...
| **Batcher**                                                              | startup: `starting_batch` and `batcher_starting_block`; <br/> operation: Trailing Batch's `CommitBatchInfo`; | none                                                                                                                 | `starting_batch`: batch **after** the last committed on L1;<br/> `batcher_starting_block`: block **after** the last block in the last committed L1 batch;<br/> Trailing `CommitBatchInfo`: Currently: not implemented, so we always start chain from zero. Later - two options: (1) Load last committed `CommitBatchInfo` from L1 OR (2) reprocess last committed batch |                             
| **Prover Input Generator**  (TODO - merged with the **Batcher** for now) |                                                                                                              | none                                                                                                                 | none                                                                                                                                                                                                                                                                                                                                                                    |                             
...
| **L1 Committer**                                                         | none*                                                                                                        | none (TODO - temporary stores `CommitBatchInfo` because it's needed for the previous `StoredBatchInfo`)              | none - recovers itself from L1                                                                                                                                                                                                                                                                                                                                          |                             
