...
- **Run locally**: `cargo run` (requires `gzip -dfk ./local-chains/v30.2/default/zkos-l1-state.json.gz && anvil --load-state ./local-chains/v30.2/default/zkos-l1-state.json --port 8545` running first)
...
1. Decompress anvil state: `gzip -dfk ./local-chains/v30.2/default/zkos-l1-state.json.gz`
2. Start anvil: `anvil --load-state ./local-chains/v30.2/default/zkos-l1-state.json --port 8545`
3. Run server: `cargo run`
4. To restart chain: `rm -rf db/*` then restart anvil
