    ...
    Finality: ReadFinality + Clone,
{
    ...
    pub fn spawn(self, runtime: &Runtime) {
        let mut priority_tree_manager = self.priority_tree_manager;
        ...
        runtime.spawn_critical_with_graceful_shutdown_signal(
            ...
            |shutdown| async move {
                ...
                tokio::select! {
                    result = priority_tree_manager.init() => {
                        result.expect("priority tree init failed");
                    }
                    _guard = &mut shutdown => {
                        return;
                    }
                ...
                let priority_tree_manager_for_prepare = priority_tree_manager.clone();
                let priority_tree_manager_for_caching = priority_tree_manager;
