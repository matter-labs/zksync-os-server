...
/// In-memory cache of `TreeBlock`s flowing through the pipeline, shared with
/// the L1 persist watcher so the watcher can recompute a batch's
/// `state_commitment` and `commitment` locally on `BlockExecution`.
///
/// `TreeBlock`s are cheap to clone (Arc-wrapped inside), so the cache value
/// uses `Arc<TreeBlock>` to keep `insert`/`get_range` allocation-free.
#[derive(Clone, Default)]
pub struct TreeBlockCache {
    inner: Arc<Mutex<HashMap<u64, Arc<TreeBlock>>>>,
}

impl TreeBlockCache {
    pub fn insert(&self, block_number: u64, tree_block: Arc<TreeBlock>) {
        self.inner
            .lock()
            .expect("TreeBlockCache mutex poisoned")
            .insert(block_number, tree_block);
    }
    ...
    pub fn get_range(&self, block_range: RangeInclusive<u64>) -> Option<Vec<Arc<TreeBlock>>> {
        let guard = self.inner.lock().expect("TreeBlockCache mutex poisoned");
        let mut out = Vec::with_capacity(
            (*block_range.end() - *block_range.start() + 1) as usize,
        );
...
/// Pipeline component that copies every `TreeBlock` into `TreeBlockCache`
/// before forwarding it downstream. Sits between `TreeManager` and whichever
/// component consumes `TreeBlock` next (`ProverInputGenerator` on the main
/// pipeline, `BatchVerificationResponder` / `NoOpSink` on the EN pipeline).
...
impl PipelineComponent for TreeBlockCacher {
    ...
    async fn run(
        ...
        loop {
            ...
            // Stash a cloned handle for the cache and forward the original block
            // down the pipeline unchanged.
            self.cache
                .insert(block_number, Arc::new(tree_block.clone()));
