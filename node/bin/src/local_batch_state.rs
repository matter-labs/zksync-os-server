...
///
/// Both `state_commitment` and `commitment` are computed from cached
/// `TreeBlock`s plus a state view at the batch's last block. If any block in
/// the range is not cached yet (e.g., on a fresh restart before the pipeline
/// catches up), `batch_commitments` returns `Ok(None)` and the watcher skips
/// the check.
...
impl<State: ReadStateHistory> BatchCommitmentSource for LocalBatchState<State> {
    fn batch_commitments(
        ...
        // Resolve the per-block `TreeBatchOutput` for every cached block. The
        // tree query can fail if the tree DB lost the version — propagate
        // rather than silencing.
        let tree_outputs: Vec<TreeBatchOutput> = cached
            .iter()
            .map(|tree_block| {
                let (root_hash, leaf_count) = tree_block.tree.block_end.clone().root_info()?;
                Ok::<_, anyhow::Error>(TreeBatchOutput {
                    root_hash,
                    leaf_count,
                })
            })
        ...
        let blocks_for_build: Vec<(&_, &[_], &TreeBatchOutput)> = cached
            .iter()
            .zip(tree_outputs.iter())
            .map(|(tree_block, tree_output)| {
                (
                    &tree_block.output,
                    tree_block.record.transactions.as_slice(),
                    tree_output,
                )
            })
            .collect();
