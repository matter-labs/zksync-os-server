...
use zksync_os_types::{
    FriVerificationStrategy, ProtocolSemanticVersion, ProvingStackConfiguration,
...
impl FriJobManager {
    ...
    fn verify_proof_blocking(
        ...
        match proving_config.fri.verification {
            FriVerificationStrategy::AirbenderProgramProof => {
                    ...
                    bincode::serde::decode_from_slice(proof_bytes, bincode::config::standard())
                        .map_err(|err| {
                            tracing::warn!(
                                batch_number,
                                ?err,
                                "Failed to deserialize unified-layer proof"
                            );
                            SubmitError::DeserializationFailed(err)
                        })?
                        .0;
                fri_proof_verifier::verify_fri_proof_unified(
                    expected_hash_u32s,
                    &program_proof,
                    batch_number,
                    application_end_params,
