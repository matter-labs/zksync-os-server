...
use zksync_os_types::{ProtocolSemanticVersion, ProvingConfiguration, require_proving_config};
...
pub struct JobMetadata {
    ...
    pub proving_config: &'static ProvingConfiguration,
    pub tx_count: usize,
...
impl JobBatchStats {
    pub fn new(metadata_list: &[JobMetadata]) -> Self {
        ...
        let max_batch = &metadata_list[metadata_list.len() - 1];
        ...
        JobBatchStats {
            ...
            max_batch_number: max_batch.batch_number,
            first_protocol_version: min_batch.protocol_version.clone(),
            last_protocol_version: max_batch.protocol_version.clone(),
...
impl Debug for JobBatchStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(f, ", spent in queue: {:?}", self.max_time_since_added)?;
