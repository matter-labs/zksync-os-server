...
impl SnarkJobManager {
    ...
    pub async fn submit_proof(
        ...
        let Some(proving_configs) = self
            .jobs
            .get_job_proving_configs(batch_from, batch_to)
            .await
        else {
            anyhow::bail!("race condition: some batches were completed earlier")
        };
        let Some((&first_config, remaining_configs)) = proving_configs.split_first() else {
            anyhow::bail!("SNARK proof range must not be empty")
        };
        anyhow::ensure!(
            first_config.verification_key_hash == submitted_vk_hash,
            "verification key hash mismatch for batch {batch_from}: server expects {}, prover submitted {submitted_vk_hash}",
            first_config.verification_key_hash,
        );

        for &proving_config in remaining_configs {
            anyhow::ensure!(
                std::ptr::eq(proving_config, first_config),
                "batches {batch_from}-{batch_to} do not use the same proving stack"
            );
        }
        ...
        permit.send(ProofCommand::new(
            ...
            SnarkProof::Real(RealSnarkProof { proof: payload }),
        ));
        Ok(())
    }
