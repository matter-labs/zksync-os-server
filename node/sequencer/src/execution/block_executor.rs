...
pub async fn execute_block(
    ...
    latency_tracker.set_state(SequencerState::InitializingVm);
    ...
    let seal_reason = loop {
        latency_tracker.set_state(SequencerState::WaitingForTx);
        tokio::select! {
            /* -------- deadline branch ------------------------------ */
            _ = async {
            ...
            maybe_tx = command.tx_source.next() => {
                latency_tracker.set_state(SequencerState::VmAndStorage);
                match maybe_tx {
    ...
    latency_tracker.set_state(SequencerState::Sealing);
