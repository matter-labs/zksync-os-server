...
use alloy::rpc::types::{Block, BlockOverrides, EIP1186AccountProofResponse, FeeHistory, Filter, FilterBlockOption, Header, Index, Log, SyncStatus, Transaction, TransactionReceipt, TransactionRequest};
    ...
    async fn logs(&self, filter: Filter) -> RpcResult<Vec<Log>> {
        ...
        let (from, to) = match filter.block_option {
            FilterBlockOption::AtBlockHash(block_hash) => {
                let block = resolve_block_id(Some(BlockId::Hash(block_hash.into())), &self.finality_info);
                (block, block)
            },
        ...
        for number in from..=to {
            if let Some(block) = self.repository_manager.block_receipt_repository.get_by_number(number) {
                let block_bloom = Bloom::new(block.header.logs_bloom);
                if filter.matches_bloom(block_bloom) {
                    let receipts = self.repository_manager.transaction_receipt_repository.get_by_block_number(number);
