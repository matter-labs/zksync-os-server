...
pub async fn run_jsonrpsee_server(
    ...
    rpc.merge(
        EthNamespace::new(state_handle, mempool, block_replay_storage, config.clone()).into_rpc(),
    )?;

    let server_builder = ServerBuilder::default().max_connections(1000u32);
...
pub fn resolve_block_id(block: Option<BlockIdVariant>, state_handle: StateHandle) -> u64 {
    ...
    match block_id {
        ...
        | BlockId::Number(BlockNumber::L1Committed) => state_handle.last_canonized_block_number(),
