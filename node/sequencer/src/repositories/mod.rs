...
impl RepositoryManager {
    ...
    fn populate_in_memory(&self, mut block_output: BatchOutput, transactions: Vec<ZkTransaction>) {
        ...
        let account_properties_latency_observer = REPOSITORIES_METRICS.insert_block_in_memory[&"account_properties"].start();
        self.account_property_repository.add_diff(block_number, account_properties);
        let account_properties_latency = account_properties_latency_observer.observe();


        // Add bytecodes to the bytecode repository
        let bytecodes_latency_observer = REPOSITORIES_METRICS.insert_block_in_memory[&"bytecodes"].start();
        ...
        let transaction_receipts_latency_observer = REPOSITORIES_METRICS.insert_block_in_memory[&"transaction_receipts"].start();
        ...
        let block_receipt_latency_observer = REPOSITORIES_METRICS.insert_block_in_memory[&"block_receipts"].start();
        self.block_receipt_repository.insert(&block_header, tx_hashes);
        ...
        REPOSITORIES_METRICS.insert_block_in_memory_per_tx.observe(latency.div(tx_count as u32));

        tracing::debug!("Stored a block {} in memory with {} transactions. Latencies: total: {:?}, account properties: {:?}, bytecodes: {:?}, transaction receipts: {:?}, block receipts: {:?}",
            block_number,
            tx_count,
            latency,
    ...
    pub async fn run_persist_loop(&self) {
        loop {
            ...
            REPOSITORIES_METRICS.persist_block_per_tx.observe(persist_latency.div(txs.len() as u32));
            ...
            REPOSITORIES_METRICS.persistence_lag.set(self.latest_block.borrow().saturating_sub(number) as usize);
