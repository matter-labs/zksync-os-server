...
/// Note that to get StorageView for block n+1 one needs data for block `n` (be it persistent_storage_map or diffs)
#[derive(Debug, Clone)]
pub struct StorageMap {
    /// block → diff map (~last  blocks).
...
impl StorageMap {
    ...
    pub fn compact(&self) {
        let can_compact_until = self
            .max_block_in_diffs()
            .saturating_sub(self.blocks_to_retain as u64);
