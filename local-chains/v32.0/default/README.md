...
The snapshot has since been upgraded in place so the chain can verify proofs from the
ZKsync OS 0.4 proving stack, which the original deployment could not:

- `ZKsyncOSVerifierPlonk` for the v32.0 VK deployed and registered on the chain's
  `ZKsyncOSDualVerifier` at **verifier selector 8** — the selector the server encodes in
  `_proof[0]` for this stack. Selector 0 still holds the ZKsync OS 0.3 verifier.
