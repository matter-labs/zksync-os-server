...
// This brings `ForwardSystemVersion` into scope as a `pub enum`.
include!(concat!(env!("OUT_DIR"), "/protocol_config_generated.rs"));

/// Look up the forward system version for a given protocol version.
...
pub fn forward_system_version(
    version: &ProtocolSemanticVersion,
) -> Result<ForwardSystemVersion, ProtocolConfigError> {
    forward_system_version_impl(version.minor, version.patch)
...
/// `forward_system_version` or protocol version directly instead.
...
mod tests {
    ...
    #[test]
    fn forward_system_version_mapping() {
        use ForwardSystemVersion::*;
        ...
        for ((major, minor, patch), expected) in test_vector.iter() {
            ...
            let fs_ver = forward_system_version(&version)
                .unwrap_or_else(|e| panic!("Failed to convert version {version:?}: {e}"));
            assert_eq!(fs_ver, *expected);
        }

        let unknown_versions = [(0, 27, 10), (0, 28, 5), (0, 33, 0)];
        for (major, minor, patch) in unknown_versions.iter() {
            let version = ProtocolSemanticVersion::new(*major, *minor, *patch);
            assert!(forward_system_version(&version).is_err());
        }
    }

    #[test]
    fn forward_system_version_u32_roundtrip() {
        for id in [1u32, 2, 3, 4, 5, 6] {
            let ev = ForwardSystemVersion::try_from(id).unwrap();
            assert_eq!(u32::from(ev), id);
        }
        assert!(ForwardSystemVersion::try_from(0u32).is_err());
        assert!(ForwardSystemVersion::try_from(99u32).is_err());
    }
