...
pub use self::proving_registry::{
    AggregationGroup, FriProofConfiguration, FriPublicInputLayout, FriVerificationStrategy,
    ProverInputStrategy, ProvingRegistry, ProvingRegistryEntry, ProvingStackConfiguration,
...
mod tests {
    ...
    #[test]
    fn registered_protocol_versions_are_supported_for_proving() {
        for entry in super::proving_registry().iter() {
            assert!(entry.protocol_version.is_supported_for_proving());
        }
    }
