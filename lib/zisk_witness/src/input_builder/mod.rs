...
pub(crate) struct AccountProofs {
    pub preimages: Vec<(Address, Vec<u8>)>,
    pub storage: Vec<(B256, StorageProof)>,
}

/// What the pre-execution pass discovered the block will read.
pub(crate) struct PreExecutionReads {
    pub keys: HashSet<B256>,
    pub addresses: HashSet<Address>,
    /// `(account, slot, value)` for every storage slot the pass touched.
...
/// State loaded for the pre-execution pass.
pub(crate) struct LoadedState {
    pub accounts: HashMap<Address, AccountInfo>,
    /// Keccak-keyed, as the EVM addresses them.
    pub bytecodes: HashMap<B256, Bytecode>,
    /// Blake2s-keyed full preimages, as the guest addresses them.
    pub preimages: Vec<(B256, Vec<u8>)>,
}
...
pub fn build_block_witness<ReadState: ReadStateHistory>(
    ...
    let LoadedState {
        accounts: mut accounts_map,
        bytecodes: mut bytecodes_map,
        preimages: mut bytecodes_out,
    } = load_accounts_and_bytecodes(
        &initial_addrs,
    ...
    for iteration in 0..max_iterations {
        ...
        let PreExecutionReads {
            keys: read_keys,
            addresses: extra_addrs,
            storage: storage_reads,
            preimages: pre_exec_preimages,
        } = pre_execute_for_reads(
            ctx,
            spec_id,
            basefee,
            prev_randao,
    ...
    let AccountProofs {
        preimages: account_preimages,
        storage: mut storage_proofs,
