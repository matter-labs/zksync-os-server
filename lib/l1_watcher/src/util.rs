...
use zksync_os_provider::network::EthereumLike;
use zksync_os_provider::{Bridgehub, MessageRoot, NodeProvider, ZkChain};
...
pub async fn find_block_by_migration_number<N: Network>(
    zk_chain: ZkChain<N>,
...
pub async fn find_l1_block_by_predicate<N: Network, Fut: Future<Output = anyhow::Result<bool>>>(
    zk_chain: Arc<ZkChain<N>>,
    start_block_number: BlockNumber,
    predicate: impl Fn(Arc<ZkChain<N>>, u64) -> Fut,
) -> anyhow::Result<BlockNumber> {
...
/// if there is not any.
async fn find_last_matching_event<E: SolEvent + Debug, N: Network>(
    address: Address,
...
async fn find_latest_l1_revert<N: Network>(
    zk_chain: &ZkChain<N>,
    batch_number: u64,
    start_block_number: BlockNumber,
    max_blocks_to_scan: u64,
) -> anyhow::Result<Option<BlockNumber>> {
    find_last_matching_event::<IExecutor::BlocksRevert, N>(
        *zk_chain.address(),
        zk_chain.provider(),
        start_block_number,
        max_blocks_to_scan,
...
pub async fn find_l1_commit_block_by_batch_number<N: Network>(
    zk_chain: ZkChain<N>,
    batch_number: u64,
    max_l1_blocks_to_scan: u64,
) -> anyhow::Result<BlockNumber> {
    let is_batch_committed = move |zk: Arc<ZkChain<N>>, block: BlockNumber| async move {
...
///
/// Returns latest L1 block is there is none.
pub async fn find_l1_execute_block_by_batch_number<N: Network>(
    zk_chain: ZkChain<N>,
    batch_number: u64,
) -> anyhow::Result<BlockNumber> {
...
pub async fn find_l1_block_by_interop_root_id<N: Network>(
    bridgehub: Bridgehub<N>,
    ...
    let predicate = async |message_root: Arc<MessageRoot<N>>, block: u64| -> anyhow::Result<bool> {
...
/// the given L1 block.
pub async fn fetch_stored_batch_data<N: EthereumLike>(
    zk_chain: &ZkChain<N>,
...
pub async fn fetch_committed_batch_data<N: EthereumLike>(
    zk_chain: &ZkChain<N>,
    tx_hash: TxHash,
