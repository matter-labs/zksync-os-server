...
///
/// Block boundaries are pre-resolved by the caller — callers that anchor on batch numbers should
/// call [`crate::util::find_l1_commit_block_by_batch_number`] /
/// [`crate::util::find_l1_execute_block_by_batch_number`] to compute them, then construct the
/// segment.
#[derive(Clone, Debug)]
pub struct SegmentSpec {
    ...
    /// First L1/SL block to scan from, inclusive.
    pub start_block: BlockNumber,
    /// Last L1/SL block to scan, inclusive. `None` means open-ended (tailed against the SL's
    /// finalized boundary).
    ...
    pub fn new(
        ...
        for seg in &segments[..segments.len() - 1] {
            anyhow::ensure!(
                ...
                "non-final SlAwareL1Watcher segments must be closed (`end_block = Some(_)`)"
            );
        }
...
async fn run_segment(
    ...
    tracing::info!(
        ...
        "sl-aware watcher activated segment",
    );
