            ...
            match f().await {
                ...
                Err(err) if err.is_retriable() => {
                    if retries > max_retries {
                        tracing::warn!(
                            ?err,
                            "Exhausted {max_retries} retries performing request; returning last error"
                        );
                        break Err(err);
                    }
                    ...
                    let sleep_duration = Duration::from_secs(backoff_secs)
                        .mul_f32(rand::rng().random_range(0.8..1.2));
...
impl<S: ObjectStore> ObjectStore for StoreWithRetries<S> {
    ...
    async fn put_raw(
        ...
        let result = Request::Put(bucket, key)
            ...
            .await;
        latency.observe();
        OBJECT_STORE_METRICS.payload_size[&bucket.0].observe(value.len());
        OBJECT_STORE_METRICS.read_write_ops[&"write"].inc();
        result
    }
