...
pub struct ZiskLaneMetrics {
    ...
    /// The `multi_proof_wait_timeout` elapsed for a batch/range whose ZiSK
    /// proof path was still unavailable. The batch is NOT degraded to an
    /// Airbender-only submission (a type-2 proof the deployed type-5
    /// MultiProofVerifier would reject, which would strand the batch): it is
    /// kept queued and re-offerable. This counter is the "timeout fired while
    /// still blocked" signal — page on sustained growth.
