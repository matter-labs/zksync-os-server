                ...
                match tx.envelope() {
                    ZkEnvelope::System(envelope) => {
                        ...
                        if let Some(roots) = envelope.interop_roots() {
                            for root in roots {
                                // Legacy (<= v31) executor fold — no timestamp word. v32 batches
                                // never take this path: their rolling hash comes from the native
                                // batch run, which mirrors the timestamped
