...
impl PendingBatchInfo {
    ...
    /// Batch output hash exactly as the zksync-os 0.4.0 batch program computes it
    /// (`BatchOutput::hash` in `basic_bootloader/.../post_tx_op/public_input.rs`): unlike the
    /// pre-v32 [`Self::public_input_hash`] layout, it does NOT include the leading `chain_id` —
    /// the chain id is committed through the chain config hash in the outer batch public input
    /// instead. Used for server-side verification of v32 FRI proofs and as the v32 arm of
